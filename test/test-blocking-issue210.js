...
let startTime1; let startTime2; let totalTime1; let totalTime2; let dropTable = 0;
const elapsedTime = informixdb.getElapsedTime;
const ret = pool.init(connectCount, connectionString);
...
var testLongTime = function (conn) {
  ...
  const query1 = 'select c1, c2 from mtab1 where (c1 in (?, ?) or c2 in (?, ?) and (? is null or c2 = ?))';
  const query2 = "select c1, c2 from mtab1 where (c1 in ('330107196906080910', '330107196906080910') " +
                 "or c2 in ('330107196906080910', '330107196906080910') and ('' is null or c2 = ''))";
  const params = ['330107196906080910', '330107196906080910', '330107196906080910', '330107196906080910', '', ''];
  startTime1 = new Date();
